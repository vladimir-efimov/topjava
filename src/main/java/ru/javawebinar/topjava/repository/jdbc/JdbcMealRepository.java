...
public class JdbcMealRepository implements MealRepository {
    ...
    public Meal save(Meal meal, int userId) {
        ...
        if (meal.isNew()) {
            ...
            meal.setId(newKey.intValue());
                ...
                "SET description=:description, calories=:calories, datetime=:datetime WHERE id=:id", map) == 0) {
            return null;
        }
        return meal;
    }
    ...
    public Meal get(int id, int userId) {
        return jdbcTemplate.queryForObject("SELECT * FROM meals WHERE id=? and user_id=?",
